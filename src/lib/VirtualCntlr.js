class VirtualCntlr {
	...
	destroy() {
		...
		let html = document.querySelector(`.${this.settings.prefix}-cntlr`);
	...
	setStyles() {
		let css = `
			...
			.${this.settings.prefix}-cntlr .btns.is-start .${this.settings.prefix}__start { fill: ${this.settings.styles.colors[1]}; }`;
